...
function loadFriends() {
    ...
    Promise.all(user.friends.map(async f => {
        ...
        return d ? { name: f, points: d.points || 0, avatar: d.avatar || '👤', avatarType: d.avatarType, avatarData: d.avatarData } : null;
    })).then(friends => {
        ...
        friendsDiv.innerHTML = valid.map(f => {
            ...
            if (f.avatarType === 'photo' && f.avatarData) {
                avatarHtml = `<img src="${f.avatarData}" style="width:32px;height:32px;border-radius:50%;object-fit:cover;">`;
            } else {
                avatarHtml = `<span style="font-size:24px;">${f.avatar || '👤'}</span>`;
            }
        ...
        leaderboardDiv.innerHTML = all.map((f, i) => {
            let avatarHtml = '';
            if (f.avatarType === 'photo' && f.avatarData) {
                avatarHtml = `<img src="${f.avatarData}" style="width:28px;height:28px;border-radius:50%;object-fit:cover;">`;
            } else {
                avatarHtml = `<span style="font-size:20px;">${f.avatar || '👤'}</span>`;
            }
